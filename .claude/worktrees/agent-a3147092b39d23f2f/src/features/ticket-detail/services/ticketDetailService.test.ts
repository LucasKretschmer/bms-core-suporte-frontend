...
describe('ticketDetailService', () => {
  ...
  it('getTicketById desempacota ApiResponse<T> (data.data) com shape aninhado real', async () => {
    ...
    const ticketResponse = {
      ...
      owner: { userId: 1, nome: 'João Silva', hubspotOwnerId: 999 },
      client: {
        id: 1,
        nomeFantasia: 'Acme',
    ...
    expect(mockedGet).toHaveBeenCalledWith('/api/v1/tickets/by-id/1')
  ...
  it('listTicketTimeEntries desempacota ApiResponse<T[]> (data.data)', async () => {
    ...
    mockedGet.mockResolvedValueOnce({
      data: { data: [{ id: 1 }, { id: 2 }], message: 'ok' },
    })
    const result = await listTicketTimeEntries(1)
    expect(mockedGet).toHaveBeenCalledWith('/api/v1/tickets/1/time-entries')
