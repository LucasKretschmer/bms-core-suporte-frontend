...
describe('useTimeEntryMutations', () => {
  ...
  it('create monta payload com works em ISO Z, userId e gera Idempotency-Key', async () => {
    vi.mocked(service.createManualTimeEntry).mockResolvedValue({ id: 'e1' })
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const { result } = renderHook(() => useTimeEntryMutations('t1'), {
    ...
    expect(payload.ticketId).toBe('t1')
  ...
  it('create invalida as queries do detalhe no sucesso', async () => {
    ...
    await waitFor(() => {
      expect(spy).toHaveBeenCalledWith({ queryKey: ['ticket-time-entries', 't1'] })
      expect(spy).toHaveBeenCalledWith({ queryKey: ['ticket-detail', 't1'] })
    })
  })

  it('update chama o service com id e payload sem userId', async () => {
    vi.mocked(service.updateManualTimeEntry).mockResolvedValue({ id: 'e1' })
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const { result } = renderHook(() => useTimeEntryMutations('t1'), {
      wrapper: wrapper(client),
    })

    await result.current.update.mutateAsync({ id: 'e1', values: form({ billableOutsidePlan: true }) })
  ...
  it('remove chama deleteTimeEntry com o id', async () => {
    ...
    await result.current.remove.mutateAsync('e1')
    expect(service.deleteTimeEntry).toHaveBeenCalledWith('e1')
  })
})
