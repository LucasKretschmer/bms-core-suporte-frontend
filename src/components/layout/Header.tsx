...
export function Header({ onMenuToggle, isSidebarCollapsed }: HeaderProps) {
  ...
  return (
    <header className="h-14 bg-card rounded-card shadow-card flex items-center justify-between px-4 gap-4 shrink-0">
      {/* Botão de toggle do sidebar */}
      <button
        type="button"
        onClick={onMenuToggle}
        aria-label={isSidebarCollapsed ? 'Expandir menu' : 'Recolher menu'}
        className="text-foreground/60 hover:text-foreground transition-colors rounded focus-visible:ring-2 focus-visible:ring-primary-light focus-visible:ring-offset-2"
      >
